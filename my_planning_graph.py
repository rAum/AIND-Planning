...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for action in self.a_levels[previous_level]:
            new_s_level.update(action.effnodes) # insert all states added by an action
            for state in action.effnodes: # for each generated by an action correctly make connection:
                state.parents.add(action)  # - add action as parent of a state
                action.children.add(state) # - add this state as child of an action

        self.s_levels.append(new_s_level)  # finally wec can append the new s level
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        """
        # the older version
        #return any(True for e1 in node_a1.effnodes if (e1.symbol, e1.is_pos) in ((e2.symbol, not e2.is_pos) for e2 in node_a2.effnodes)))
        # the more optimized version from review comment
        return (set(node_a1.action.effect_add) & set(node_a2.action.effect_rem) ) or ( set(node_a1.action.effect_add) & set(node_a2.action.effect_rem) )
